...
exports.main = async () => {
  try {
    ...
    if (countResult.total > 0) {
      return { success: true, message: `数据库已有 ${countResult.total} 条数据，跳过初始化` };
    }

    const batchSize = 10;
    for (let i = 0; i < SAMPLE_DISHES.length; i += batchSize) {
      const batch = SAMPLE_DISHES.slice(i, i + batchSize);
      await Promise.all(batch.map((dish) =>
        db.collection('dishes').add({ data: dish })
      ));
    }

    return { success: true, message: `成功导入 ${SAMPLE_DISHES.length} 道菜` };
  } catch (err) {
